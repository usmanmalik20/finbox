...
SideBar = (function($, window) {
    ...
    config = {shortenOnClickOutside: true, animation: false};
    ...
    expadSidebar = function() {
        ...
        if (config.animation === true) {
            ...
            chatUsers.find('.user-filter .input-filter').stop().animate({width: "143"}, anmationDuration * 2);
            chatUsers.find('.user-list li a i.user-status').stop().animate({marginRight: '14', marginTop: '4'}, anmationDuration * 2);
            sidebarScroll.find('.menu .badge').stop().animate({marginRight: '5'}, anmationDuration * 2);
            socialBarContainer.find(".user .trigger-user-settings").stop().animate({marginRight: "15"}, anmationDuration);
            $("#main").stop().animate({marginRight: marginRight, marginLeft: "200"}, anmationDuration);
            $(".social-navbar").stop().animate({left: "200"}, anmationDuration, function() {
    ...
    reduceSidebar = function() {
        ...
        if (config.animation === true) {
            ...
            chatUsers.find('.user-filter .input-filter').stop().animate({width: "0"}, anmationDuration * 2);
            chatUsers.find('.user-list li a i.user-status').stop().animate({marginRight: '6', marginTop: '6'}, anmationDuration * 2);
            socialBarContainer.find(".user .trigger-user-settings").stop().animate({marginRight: "-12"}, anmationDuration);
            $("#main").stop().animate({marginLeft: "54", marginRight: "0"}, anmationDuration * 2, 'swing', function() {
                return $("body").css("overflow-x", "auto");
            });
            $(".social-navbar").stop().animate({left: "54"}, anmationDuration * 2, 'swing', function() {
                return socialBarContainer.find(".search-sidebar img").stop().fadeIn(anmationDuration * 2);
            });
            sidebarScroll.find('.menu .badge').stop().animate({marginRight: '-4', marginLeft: "-12"}, anmationDuration * 2);
    ...
    handleSideBarScroll = function() {
        ...
        sidebarScrollOptions = {height: $(window).height() - chatUsers.outerHeight(), size: "8px", railVisible: true, railColor: "#000"};
    ...
    handleSidebarChatScroll = function() {
        $(".user-list").slimscroll({height: chatUsers.find(".user-list").height(), size: "8px", railColor: "#000", wheelStep: 15});
    };
    handleSidebarChatUsersResize = function() {
        if (!$().resizable) {
            return;
        }
        chatUsers.resizable({handles: "n", maxHeight: 535, minHeight: 110, resize: function(event, ui) {
    ...
    handleSidebarChatUsersFilter = function() {
        ...
        chatUsers.find(".user-list").liveFilter(".chat-users .user-filter input.input-filter", "li", {filterChildSelector: "a", after: function() {
    ...
    return{init: init, isTouchDevice: isTouchDevice};
})(jQuery, window);