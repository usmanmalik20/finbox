...
Charts = (function($) {
    ...
    handleBasicChart = function() {
        ...
        $.plot($("#basic-charts"), [{data: d1, lines: {show: true, fill: true}, color: "#5bb75b"}, {data: d2, bars: {show: true}, color: "#4e6599"}, {data: d3, points: {show: true}, color: "#49afcd"}, {data: d4, lines: {show: true}, color: "#faa732"}, {data: d5, lines: {show: true}, color: "#da4f49", points: {show: true}}, {data: d6, lines: {show: true, steps: true}, color: "#363636"}]);
    };
    handleChartWithSymbols = function() {
        ...
        data = [{data: generate(2, 1.8), points: {symbol: "circle"}, color: "#4e6599"}, {data: generate(3, 1.5), points: {symbol: "square"}, color: "#49afcd"}, {data: generate(4, 0.9), points: {symbol: "diamond"}, color: "#51a351"}, {data: generate(6, 1.4), points: {symbol: "triangle"}, color: "#faa732"}, {data: generate(7, 1.1), points: {symbol: "cross"}, color: "#da4f49"}];
        $.plot($("#symbols-chart"), data, {series: {points: {show: true, radius: 3}}, grid: {hoverable: true}});
    };
    handleDonutPieChart = function() {
        ...
        $.plot($("#pie-chart"), data, {series: {pie: {show: true, radius: 1, label: {show: true, radius: 3 / 4, formatter: function(label, series) {
                            return"<div style=\"font-size:8pt;text-align:center;padding:2px;color:white;\">" + label + "<br/>" + Math.round(series.percent) + "%</div>";
                        }, background: {opacity: 0.5, color: "#000"}}}}, legend: {show: false}});
        $.plot($("#donut-chart"), data, {series: {pie: {innerRadius: 0.5, show: true}}});
    };
    handleBarChart = function() {
        ...
        plotWithOptions = function() {
            return $.plot($("#bar-chart"), [d1, d2, d3], {series: {stack: stack, lines: {show: lines, fill: true, steps: steps}, bars: {show: bars, barWidth: 0.6}}});
        };
        plotWithOptions();
    ...
    handleLiveChart = function() {
        ...
        options = {series: {shadowSize: 0, lines: {fill: true}}, yaxis: {min: 0, max: 100}, xaxis: {show: false}, grid: {backgroundColor: {colors: ["#fff", "#e4f4f4"]}}};
